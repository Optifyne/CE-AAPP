...
function CEchangeStatistic() {
    var ChangeStatisticAction = Java.extend(ConditionalEventsAction, {
        execute: function(player, actionLine, minecraftEvent) {
            ...
            if (args.length < 5) {
                Bukkit.getLogger().warning("[CEActions] CHANGE_STATISTIC ACTION: Invalid actionLine format! CORRECT FORMAT: change_statistic: <player (may be offline)>;<statistic_name (for example: KILL_ENTITY)>;<argument (for example: ZOMBIE, it will not be taken into account where is not needed)>;<new_value (for example: 100)>;<operation (may be SET, ADD, REMOVE, MULTIPLY, DIVIDE or RESET)>");
                return;
            }
            ...
            if (!targetPlayer) {
                ...
                if (!targetPlayer || (targetPlayer && !targetPlayer.hasPlayedBefore())) {
