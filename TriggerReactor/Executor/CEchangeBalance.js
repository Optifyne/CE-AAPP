    ...
    if (!essentialsPlugin) {
        ...
        var ChangeBalanceAction = Java.extend(ConditionalEventsAction, {
            execute: function(player, actionLine) {
                ...
                var amount = new BigDecimal(args[1].trim());
                var operation = args[2].trim().toUpperCase();

                var targetPlayer = Bukkit.getPlayer(targetName) || Bukkit.getOfflinePlayer(targetName);
                if (!targetPlayer) {
                    Bukkit.getLogger().warning("[CEActions] CHANGE_BALANCE ACTION: Player '" + targetName + "' is not found!");
                    return;
                }
