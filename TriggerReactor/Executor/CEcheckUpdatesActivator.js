...
function httpGet(url) {
    try {
        ...
        var req = HttpRequest.newBuilder(URI.create(String(url))).timeout(Duration.ofSeconds(6)).header('User-Agent', 'Nashorn-UpdateChecker').build();
...
function buildUpdateMessage(currentMajor, currentBuildIsoDate, latestMajor, latestBuildIsoDate, downloadUrl, changelogUrl) {
    var msg = "§e[CEAAPP]§r §fNew version available: §c" + latestMajor + " §7from §f" + latestBuildIsoDate + "§7. §fYour version: " + "§a" + currentMajor + " §7(§f" + currentBuildIsoDate + "§7)§f.";
    ...
    scheduler.runTaskAsynchronously(plugin, new Runnable({
        run: function () {
            try {
                ...
                var outdated = (compareSemVer(latestMajor, String(currentMajor)) > 0) || (latestMajor === String(currentMajor) && latestDate.isAfter(currentDate));
                ...
                if (outdated) {
                    ...
                    scheduler.runTask(plugin, new Runnable({
                        run: function () {
                            Bukkit.getConsoleSender().sendMessage("");
                            Bukkit.getConsoleSender().sendMessage("§e[CEAAPP]§r §fCurrent version is up to date: §a" + currentMajor + " §7(§f" + currentBuildIsoDate + "§7)§f.");
                            Bukkit.getConsoleSender().sendMessage("");
						}
                    }));
                }
                ...
                scheduler.runTask(plugin, new Runnable({
                    run: function () {
                        Bukkit.getLogger().warning("[CEAAPP] Update check failed: " + String(err && err.message ? err.message : err));
                    }
                }));
            }
        }
