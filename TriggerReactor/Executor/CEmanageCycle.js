        ...
        run: function () {
            ...
            if (isEndForCurrent && !(willOvershoot && notAtBoundary)) {
                ...
                dispatchSync(
                    ...
                    + ";%l%=" + arrayLocal.size()
                    + ";%f%=" + (ctx.state.forward ? "+" : "-")
                    + ";%i1%=" + ctx.state.startIndex
                    + ";%i2%=" + ctx.state.i2
                    + ";%end%=true"
                    + ";%start%=" + (ctx.state.started ? "false" : "true")
                    + ";%o%=false"
                    + (ctx.state.extraVariables ? ";" + ctx.state.extraVariables.replace(/\$val@/g, val).replace(/\$ind@/g, ctx.state.i) : "")
                    + (ctx.state.playerName ? (" player:" + ctx.state.playerName.replace(/\$val@/g, val)) : "")
                    + " silent:true"
                );
                return;
            }
    ...
    var ManageCycleAction = Java.extend(ConditionalEventsAction, {
        execute: function (player, actionLine, minecraftEvent) {
            ...
            if (action === "run") {
                ...
                var extraVariables = args.length > idx + 9 ? args[idx + 9].replaceAll("`", ";") : null;
                    ...
                    if (syncImmediate) {
                        ...
                        for (var i = i1; (forward ? (i <= i2) : (i >= i2)); i += step) {
                            ...
                            dispatchSync(
                                ...
                                + ";%o%=false"
                                + (extraVariables ? ";" + extraVariables.replace(/\$val@/g, val).replace(/\$ind@/g, i) : "")
                                + (player ? (" player:" + player.replace(/\$val@/g, val)) : "")
                                + " silent:true"
                            );
                        ...
                        new (Java.extend(Runnable, {
                            run: function () {
                                ...
                                dispatchSync(
                                    ...
                                    + ";%i1%=" + ctxS.state.startIndex
                                    + ";%i2%=" + ctxS.state.i2
                                    + ";%end%=" + (isEndForCurrent ? "true" : "false")
                                    + ";%start%=" + (ctxS.state.started ? "false" : "true")
                                    + ";%o%=false"
                                    + (ctxS.state.extraVariables ? ";" + ctxS.state.extraVariables.replace(/\$val@/g, val).replace(/\$ind@/g, ctxS.state.i) : "")
                                    + (ctxS.state.playerName ? (" player:" + ctxS.state.playerName.replace(/\$val@/g, val)) : "")
                                    + " silent:true"
                                );
