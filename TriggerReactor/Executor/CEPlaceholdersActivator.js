    ...
    var CEPlaceholders = Java.extend(PlaceholderExpansion, {
        ...
        onPlaceholderRequest: function(player, identifier) {
            ...
            if (identifier.startsWith("item_")) {
                ...
                var mode = args[0];
                var slotAndInventory = mode !== "slot" ? args[1].replaceAll("-", "_").split("|") : null;
                var targetIdentifier = args[mode !== "slot" ? 2 : 1] && args[mode !== "slot" ? 2 : 1] !== "" ? args.slice(mode !== "slot" ? 2 : 1).join("_") : player.getName();
                ...
                if (targetIdentifier.indexOf(",") !== -1) {
                    ...
                    if (parts.length === 4) {
                        ...
                        if (world) {
                            target = world.getBlockAt(x, y, z).getState();
                        }
                    }
                } else {
                    try {
                ...
                if (mode !== "slot") {
                    var opened = slotAndInventory && slotAndInventory.length > 1 && slotAndInventory[0].toUpperCase() === "OPENED";
                    var slot = slotAndInventory ? opened ? slotAndInventory[1] : slotAndInventory[0] : null;
                    var item = null;
                    var validSlots = ["HEAD", "CHEST", "LEGS", "FEET", "HAND", "OFF_HAND"];
                }

                if (target instanceof Player) {
                    if (mode === "slot") return target.getInventory().getHeldItemSlot();
                    var inventory = opened ? target.getOpenInventory().getTopInventory() : target.getInventory();
                    if (slot !== null && slot.toUpperCase() === "CURSOR") {
                        item = target.getItemOnCursor();
                    } else {
                        if (slot !== null && ((!isNaN(slot) && (slot < 0 || slot >= inventory.getSize())) || (isNaN(slot) && validSlots.indexOf(slot.toUpperCase()) === -1))) return "Invalid Slot";
                        item = slot !== null ? isNaN(slot) ? inventory.getItem(EquipmentSlot.valueOf(slot.toUpperCase())) : inventory.getItem(parseInt(slot)) : inventory.getItemInMainHand();
                    }
                } else if (target.getEquipment) {
                    if (mode === "slot") return "0";
                    var inventory = target.getEquipment();
                    if (slot !== null && validSlots.indexOf(slot.toUpperCase()) === -1) return "Invalid Equipment Slot";
                    item = slot !== null ? inventory.getItem(EquipmentSlot.valueOf(slot.toUpperCase())) : inventory.getItemInMainHand();
                } else if (target.getInventory) {
                    if (mode === "slot") return "0";
                    var inventory = target.getInventory();
                    if (slot !== null && (isNaN(slot) || (!isNaN(slot) && (slot < 0 || slot >= inventory.getSize())))) return "Invalid Slot";
                    item = slot !== null ? inventory.getItem(parseInt(slot)) : inventory.getItem(0);
                }
                
                if (!inventory) return "Invalid Inventory";
                ...
                if (mode.startsWith("lore")) {
                    ...
                    return selectedLore.trim() || "Invalid Index";
                }
                ...
                switch (mode) {
                    case "slot":
                        return slot !== null ? slot : "MainHand";
                    case "enchantments":
            ...
            if (identifier.startsWith("lp_has_permission_in_contexts_") || identifier.startsWith("lp_get_permission_contexts_")) {
                if (!luckpermsPlugin) {
                	return "[CEPlaceholders] LP PLACEHOLDER: Placeholder was skipped because LuckPerms is missing!";
            	} else {
                    ...
                    if (parts.length >= 2) {
                        ...
                        var permission = parts[parts.length - 2];
                        var contextString = parts[parts.length - 1] ? parts[parts.length - 1] : null;
            ...
            if (identifier.startsWith("highest_block_player_")) {
                var playerName = identifier.substring("highest_block_player_".length());
            ...
            if (identifier.startsWith("highest_block_uuid_")) {
                var uuidString = identifier.substring("highest_block_uuid_".length());
                if (uuidString === "") return "Invalid UUID";
            ...
            if (identifier.startsWith("highest_block_coords_")) {
                var location = identifier.substring("highest_block_coords_".length()).split(",");
                if (location.length < 3) return "Invalid location";
            ...
            if (identifier.startsWith("random_block_")) {
                var blocksIndex = identifier.indexOf("blocks=");

                var parts = [];
                if (blocksIndex !== -1) {
                    parts = identifier.substring("random_block_".length, blocksIndex).split("_");
                } else {
                    parts = identifier.substring("random_block_".length).split("_");
                }
            ...
            if (identifier.startsWith("entitiesInRadius_")) {
                ...
                var x = parseInt(args[args.length - 6]);
                var y = parseInt(args[args.length - 5]);
                var z = parseInt(args[args.length - 4]);
                var radius = parseInt(args[args.length - 3]);
            ...
            if (identifier.startsWith("entitiesListInRadius_")) {
                ...
                var x = parseInt(args[args.length - 7]);
                var y = parseInt(args[args.length - 6]);
                var z = parseInt(args[args.length - 5]);
                var radius = parseInt(args[args.length - 4]);
            ...
            if (identifier.startsWith("entityAtIndexInRadius_")) {
                ...
                var x = parseInt(args[args.length - 8]);
                var y = parseInt(args[args.length - 7]);
                var z = parseInt(args[args.length - 6]);
                var radius = parseInt(args[args.length - 5]);
            ...
            if (identifier.startsWith("entity_in_water")) {
                var args = identifier.split("_");
                var targetEntity = null;
                if (args.length <= 3) {
                    return player.isInWater() ? "true" : "false";
                } else {
                    var entityIdentifier = args.slice(3).join("_");
                    if (entityIdentifier.match(/^([0-9a-fA-F-]{36})$/)) {
                        var uuid = entityIdentifier;
                        targetEntity = Bukkit.getEntity(UUID.fromString(uuid));
                    } else {
                        targetEntity = Bukkit.getPlayer(entityIdentifier);
                    }
                    
                    if (targetEntity != null && targetEntity instanceof Entity) {
                        return targetEntity.isInWater() ? "true" : "false";
                    }
                }
            ...
            if (identifier.startsWith("realname")) {
                if (!essentialsPlugin) {
                	return "[CEPlaceholders] REALNAME PLACEHOLDER: Placeholder was skipped because EssentialsX is missing!";
            	} else {
            ...
            if (identifier.startsWith("currentHealth_") || identifier.startsWith("maxHealth_")) {
                var getMax = identifier.startsWith("currentHealth_") ? false : true;
                var targetIdentifier = getMax ? identifier.substring("maxHealth_".length) : identifier.substring("currentHealth_".length);
                ...
                return getMax ? target.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue().toFixed(2) : target.getHealth().toFixed(2);
            }
            
            if (identifier.startsWith("inventory")) {
                var targetIdentifier = identifier.startsWith("inventory_") ? identifier.substring("inventory_".length) : player.getName();

                var target = Bukkit.getPlayer(targetIdentifier);
                
                if (!target) return "PlayerNotFound";
                    
                return target.getOpenInventory().getType();
            }
            
            if (identifier.startsWith("passengers_")) {
                var targetIdentifier = identifier.substring("passengers_".length);
            ...
            if (identifier.startsWith("custom_data_")) {
                var args = identifier.substring("custom_data_".length).split("_");
            ...
            if (identifier.startsWith("onfire_")) {
				var args = identifier.substring("onfire_".length).split("_");
                var state = args[0];
            ...
            if (identifier.startsWith("frozen")) {
                var targetIdentifier = identifier.startsWith("frozen_") ? identifier.substring("frozen_".length) : player.getName();
            ...
            if (identifier.startsWith("location_")) {
				var args = identifier.substring("location_".length).split("_");
                var coords = args[0] ? args[0].split(",") : null;
                var targetIdentifier = args.length > 1 && args[1] !== "" ? args.slice(1).join("_") : player.getName();
                
                if (!coords) return "InvalidLocation";
