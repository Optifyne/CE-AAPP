...
var SuperiorSkyblock = Bukkit.getPluginManager().getPlugin("SuperiorSkyblock2");

// Custom data placeholder variables
var customDataTempGlobalData = new HashMap();
var customDataTempTargetsData = new HashMap();
var customDataFileLock = new java.lang.Object();
var customDataStorageFilePath = "plugins/TriggerReactor/customData.json";
var oldCustomDataStorageFilePath = "plugins/TriggerReactor/var.json";

// Cache variables and tasks lists variables
...
// Thread pools and scheduler variables
...
// To check whether the plugin is enabled or disabled
...
// Get entities in certain area
function getEntitiesInArea(world, x, y, z, radiusX, radiusY, radiusZ, include, filters, pl) {
    var location = new Location(world, x, y, z);
    var entities = world.getNearbyEntities(location, radiusX, radiusY, radiusZ).stream();

    if (filters == null || filters.length === 0) {
        return entities.filter(function(entity) { if (!include && entity === pl) return false; return true; }).collect(Collectors.toList());
    }
    ...
    return entities.filter(function(entity) {
        if (filters.some(function(filter) {
            ...
            if (newFilter === "PLAYER" && newFilter === entity.getType().toString()) {
                if (include && entity == pl) return true;
                if (!include && entity != pl) return true;
                return false;
            }
        ...
        if ((entity instanceof Player) && !include && entity === pl) return false;
...
function syncGetEntitiesInArea(world, x, y, z, radiusX, radiusY, radiusZ, include, filters, pl) {
    if (Bukkit.isPrimaryThread()) {
        return getEntitiesInArea(world, x, y, z, radiusX, radiusY, radiusZ, include, filters, pl);
    }
    ...
    var runnable = new Runnable({
        run: function () {
            try {
                result.set(getEntitiesInArea(world, x, y, z, radiusX, radiusY, radiusZ, include, filters, pl));
            } finally {
                latch.countDown();
            }
        }
...
// LP get/has permission with contexts placeholder
function processNodes(nodes, permission, requestedContexts, contextsForPermission) {
...
function getContextsForPermission(luckPerms, target, permission, contextString, isGroup) {
    ...
    if (isGroup) {
        ...
        if (group != null) {
            processNodes(group.getNodes(), permission, requestedContexts, contextsForPermission);
        }
    } else {
        ...
        if (user != null) {
            processNodes(user.getNodes(), permission, requestedContexts, contextsForPermission);
        }
    }
...
function hasPermissionWithContexts(luckPerms, target, permission, contextString, isGroup) {
...
// For array placeholder
function makeArray(str, separator) { return str ? str.replaceAll("␠", " ").split(separator) : []; }
...
function processArray(args) {
    ...
    switch (operation) {
        case "GET":
        case "G":
            array = makeArray(rest, separator);
        ...
        case "GBI":
            ...
            array = makeArray(sp.head, separator);
...
function findEndForOutput(input) {
    var i = 0, depth = 0;
    while (i < input.length) {
...
function normalizeForListCompare(x, CURRENCY_TAIL_RE) {
    if (x == null) return "";
...
function eqBySmart(a, b, CURRENCY_TAIL_RE) {
    var aa = normalizeForListCompare(a, CURRENCY_TAIL_RE);
    var bb = normalizeForListCompare(b, CURRENCY_TAIL_RE);
...
function rankByList(val, listArr, CURRENCY_TAIL_RE, INF) {
    if (!listArr || listArr.length === 0) return INF;
    for (var i = 0; i < listArr.length; i++) {
        if (eqBySmart(val, listArr[i], CURRENCY_TAIL_RE)) return i;
    }
    return INF;
}

function splitSortRulesByAnd(raw) {
    var rules = [];
    ...
    while (i < raw.length) {
        ...
        if (depth === 0 && ch === "&" && nx === "&") {
            rules.push(raw.substring(start, i).trim());
            i += 2;
            start = i;
            continue;
        }
...
function parseSortRule(ruleRaw) {
    var trimmed = ruleRaw ? ruleRaw.trim() : "";
    if (trimmed.length === 0) return null;

    var upper = trimmed.toUpperCase();
    if (upper === "AZ" || upper === "ZA") {
        return { kind: "VALUE", desc: (upper === "ZA"), placeholder: null, list: null };
    }

    var phEnd = findEndOfFormattedPlaceholderString(trimmed);
    if (phEnd === -1) return null;

    var placeholder = trimmed.substring(0, phEnd);
    var extra = trimmed.substring(phEnd).trim();
    var desc = false;
    var list = null;
    ...
    return { kind: "PLACEHOLDER", placeholder: placeholder, desc: desc, list: list };
}

function findSortToOutputSeparatorIndex(input) {
    var i = 0, depth = 0;
    while (i < input.length) {
        var ch = input.charAt(i);
        var nx = input.charAt(i + 1);
        ...
        if (ch === "$") { depth++; i++; continue; }
        if (ch === "@") { depth = Math.max(0, depth - 1); i++; continue; }
        if (depth === 0 && ch === "_" && input.charAt(i + 1) === "$") return i;
        i++;
    }
    return -1;
}
...
function processList(withoutPrefix, limit) {
    ...
    var type = afterSeparator.substring(0, typeEnd);
    ...
    if (afterType.startsWith("AZ_")) {
        ...
        var usIdx = findSortToOutputSeparatorIndex(tail);
        if (usIdx === -1) return "InvalidArguments";

        var extraRaw = tail.substring(0, usIdx);
        rest = tail.substring(usIdx + 1);

        var sortSpec = afterSort.substring(0, phEnd) + extraRaw;
        var rawRules = splitSortRulesByAnd(sortSpec);
        sortRules = [];
        for (var sr = 0; sr < rawRules.length; sr++) {
            var parsedRule = parseSortRule(rawRules[sr]);
            if (!parsedRule) return "InvalidArguments";
            sortRules.push(parsedRule);
        }
        if (sortRules.length === 0) return "InvalidArguments";
    } else {
        return "InvalidArguments";
    }

    var outputEnd = findEndForOutput(rest);
    ...
    var result = [];
    var players = (type.toUpperCase() === "ONLINE") ? Bukkit.getOnlinePlayers() : [];
    if (type.toUpperCase() === "ALL" || type.toUpperCase() === "OFFLINE") {
        var offline = Bukkit.getOfflinePlayers();
        for (var i = 0; i < offline.length; i++) {
            checkInterrupted();
            var p = offline[i];
            if (p.isOnline()) {
                if (type.toUpperCase() === "OFFLINE") continue;
            }
            players.push(offline[i]);
        }
    }

    if (type.toUpperCase().startsWith("PLAYERS:")) {
        var namesOrUUIDs = type.substring(8).split(",");
        namesOrUUIDs.forEach(function (nameOrUUID) {
            var cur = nameOrUUID.trim();
            if (cur === "") return;
            
            var p;
            try {
                p = Bukkit.getOfflinePlayer(UUID.fromString(cur));
            } catch (e) {
                try {
                    p = Bukkit.getOfflinePlayer(cur);
                } catch (e) {}
            }

            if (p && (p.hasPlayedBefore() || p.isOnline())) players.push(p);
        });
    }
    ...
    for (var i = 0; i < players.length; i++) {
        ...
        try {
            ...
            if (type.toUpperCase() !== "ONLINE") continue;
        }
    }

    if (sortMode === "AZ" || sortMode === "ZA" || (sortMode === "PLACEHOLDER" && sortRules && sortRules.length > 0)) {
        checkInterrupted();
    }
    ...
    if (sortMode === "AZ") {
    ...
    } else if (sortMode === "PLACEHOLDER" && sortRules && sortRules.length > 0) {
        var INF = 1e9;
        ...
        result.forEach(function (e) {
            ...
            e.sortVals = [];
            e.sortRanks = [];
            for (var ri = 0; ri < sortRules.length; ri++) {
                var rule = sortRules[ri];
                var v = (rule.kind === "VALUE")
                    ? e.value
                    : resolveNestedPlaceholders(e.player, rule.placeholder);
                e.sortVals.push(v);
                e.sortRanks.push((rule.list && rule.list.length > 0) ? rankByList(v, rule.list, CURRENCY_TAIL_RE, INF) : null);
            }
        });
        ...
        result.sort(function (a, b) {
            for (var ri = 0; ri < sortRules.length; ri++) {
                var rule = sortRules[ri];
                var aVal = a.sortVals[ri];
                var bVal = b.sortVals[ri];
                var aRank = a.sortRanks[ri];
                var bRank = b.sortRanks[ri];

                if (rule.list && rule.list.length > 0) {
                    var aInf = (aRank === INF);
                    var bInf = (bRank === INF);
                    if (aInf !== bInf) return aInf ? 1 : -1;
                    if (!aInf && aRank !== bRank) return rule.desc ? (bRank - aRank) : (aRank - bRank);
                }

                var aNum = parseStrictNumber(normalizeForListCompare(aVal, CURRENCY_TAIL_RE));
                var bNum = parseStrictNumber(normalizeForListCompare(bVal, CURRENCY_TAIL_RE));
                if (aNum !== null && bNum !== null) {
                    var numCmp = rule.desc ? (bNum - aNum) : (aNum - bNum);
                    if (numCmp !== 0) return numCmp;
                    continue;
                }

                var sa = (aVal == null) ? "" : aVal.toString();
                var sb = (bVal == null) ? "" : bVal.toString();
                var strCmp = rule.desc ? sb.localeCompare(sa) : sa.localeCompare(sb);
...
// Process conditions in different placeholders, such as "list" or "array"
var operators = [
...
function getTaskForUUID(uuidString, last, separator) {
...
function getEffectsFormatted(effects, includes, separator) {
    if (!includes) return effects.stream().map(function(e) { return e.getType().getName(); }).collect(Collectors.joining(separator));
    else {
        var lvls = includes.indexOf("includeLvls") !== -1;
        var times = includes.indexOf("includeTimes") !== -1;
        return effects.stream()
        	.map(function(e) { return e.getType().getName() + (lvls ? ("=" + e.getAmplifier()) : "") + (times ? ("=" + e.getDuration()) : ""); })
...
function uuidFromPlain(plain) {
    var hyphenated = plain.replaceFirst("(\\p{XDigit}{8})(\\p{XDigit}{4})(\\p{XDigit}{4})(\\p{XDigit}{4})(\\p{XDigit}+)", "$1-$2-$3-$4-$5");
    ...
    entries.forEach(function(entry) {
        ...
        if (parts.length === 1) {
            ...
            if (single.length === 4) {
                list.push({
                    negate: negate,
                    world: single[0].replaceAll("бµ•", "_"),
...
function isValidBlock(block, includeSolid, includeFluid, excludeAir, blockList, blockDataList, highestOffset) {
    ...
    var y = block.getY();
    var z = block.getZ();
    var highestY = block.getWorld().getHighestBlockYAt(x, z);
...
function isExcluded(x, y, z, worldName, excludedCoords) {
...
function withVarJsonLock(callback) {
    return Java.synchronized(function () {
        var file = new java.io.File(customDataStorageFilePath);
        var json = {};

        if (file.exists()) {
            json = loadData(file);
        } else {
            var legacyFile = new java.io.File(oldCustomDataStorageFilePath);
            if (legacyFile.exists()) {
                var legacyJson = loadData(legacyFile);
                if (
                    legacyJson &&
                    legacyJson.CEAAPP &&
                    legacyJson.CEAAPP.CEPlaceholders &&
                    legacyJson.CEAAPP.CEPlaceholders.CustomDataPlaceholder
                ) {
                    json.CEAAPP = {
                        CEPlaceholders: {
                            CustomDataPlaceholder: deepCloneJson(legacyJson.CEAAPP.CEPlaceholders.CustomDataPlaceholder)
                        }
                    };
                }
            }
        ...
        return callback(file, json);
    }, customDataFileLock)();
}

// For "permValue" placeholder
function extractMaxLimit(prefix, permissionData) {
    var max = null;
...
// For "item" placeholders
function formatOneModifier(mod, attrName) {
    var amount = (mod && mod.getAmount) ? mod.getAmount() : "NaN";
    var op = (mod && mod.getOperation) ? String(mod.getOperation()) : "UNKNOWN_OPERATION";

    var slot;
    try { if (mod && mod.getSlotGroup) { var sg = mod.getSlotGroup(); if (sg) slot = String(sg); } } catch (e) {}
    if (!slot) {
        try { if (mod && mod.getSlot) { var s = mod.getSlot(); if (s) slot = String(s); } } catch (e) {}
    }
    if (!slot) slot = "ANY";

    var key = "";
    try { if (mod && mod.getKey) { var k = mod.getKey(); if (k) key = String(k); } } catch (e) {}
    if (!key) { try { if (mod && mod.getName) key = String(mod.getName()); } catch (e) {} }

    return (attrName || "UNKNOWN_ATTRIBUTE") + ";" + amount + ";" + op + ";" + slot + ";" + key;
}

function formatAttributesMultiMap(mm, separator, key, uuid) {
    if (!separator) separator = "|";
    if (!mm || !mm.entries || mm.isEmpty()) return "NoAttributes";
    ...
    while (it.hasNext()) {
        ...
        var mod = e.getValue();
        if (key || uuid) {
            if (key && mod.getKey ? mod.getKey().equals(key) : mod.getUniqueId().equals(uuid)) {
                out.push(formatOneModifier(mod, String(e.getKey())));
                break;
            }
        } else out.push(formatOneModifier(mod, String(e.getKey())));
    }
    return out.length > 0 ? out.join(separator) : "NotFound";
}

function formatAttributeModifiersCollection(mods, separator, attributeName, key, uuid) {
    if (!separator) separator = "|";
    if (!mods || mods.isEmpty()) return "NoAttributes";
    ...
    while (it.hasNext()) {
        ...
        if (key || uuid) {
            if (key && mod.getKey ? mod.getKey().equals(key) : mod.getUniqueId().equals(uuid)) {
                out.push(formatOneModifier(mod, String(attributeName || "UNKNOWN_ATTRIBUTE")));
                break;
            }
        } else out.push(formatOneModifier(mod, String(attributeName || "UNKNOWN_ATTRIBUTE")));
    }
    return out.length > 0 ? out.join(separator) : "NotFound";
}
...
function CEPlaceholdersActivator() {
    var CEPlaceholders = Java.extend(PlaceholderExpansion, {
        ...
        onRequest: function(player, identifier) {
            ...
            if (identifier.startsWith("item_")) {
                ...
                var target = null;
                try {
                    var uuid = UUID.fromString(args[args.length - 1]);
                    target = getEntitySync(uuid);
                } catch (e) {}
                
                var item = null;
                var entityItem = null;
                if (!target || !(target instanceof Item)) {
                    target = null;
                    var restrict = mode !== "handSlot" && !mode.startsWith("cooldown:");

                    if (restrict && args.length < 3) return "InvalidArguments";
                    ...
                    entityItem = target;
                    item = target.getItemStack();
                }
                
                if (!item || item.getType().isAir()) return "ItemNotFound";
                
                if (item.getItemMeta() === null) return "ItemDoesNotHaveMetaData";
                ...
                if (mode.startsWith("NBTCustomData:")) {
                	var parts = mode.split(":");
                    mode = parts[0];
                    var attribute = parts[1].replaceAll("ᵕ", "_");
                }

                if (mode.startsWith("attributes:")) {
                	var parts = mode.split(":");
                    mode = parts[0];
                    var attributeOptions = parts.slice(1).join(":");
                }
                ...
                if (mode.startsWith("dataContainer-")) {
                    ...
                    var storedTypes = [
						...
						PersistentDataType.LONG, PersistentDataType.BOOLEAN, PersistentDataType.BYTE, PersistentDataType.SHORT,
                ...
                switch (mode) {
                    ...
                    case "currentDurability":
                        var meta = item.getItemMeta();
                        if (!(meta instanceof Damageable)) return "ItemIsNotDamageable";
                    ...
                    case "itemModel":
                        var meta = item.getItemMeta();
                        return meta.getItemModel ? (meta.getItemModel() || "None") : "Only1.21.2+";
                    case "skullOwner":
                        var meta = item.getItemMeta();
                    ...
                    case "NBTCustomData":
                        try {
                            ...
                            return "OnlyPaper1.20.5+";
                        }

                        var nmsItem = CraftItemStack.asNMSCopy(item);
                        var cd = nmsItem.get(DataComponents.CUSTOM_DATA);
                        
                        if (!cd) return "None";

                        var tag = cd.copyTag();
                        var nbt = tag.get(attribute);

                        if (nbt instanceof StringTag) return nbt.asString().orElse("None");
                        return nbt !== null ? nbt : "None";
                    case "attributes":
                        var meta = item.getItemMeta();
                        if (!attributeOptions) return formatAttributesMultiMap(meta.getAttributeModifiers(), separator, null, null);

                        var args = attributeOptions.split(":");
                        if (args.length < 3) return "InvalidParameters";

                        var key = args[2].trim();
                        var nsKey, targetUUID;
                        if (key !== "") {
                            try {
                                var newKey;
                                if (key.contains("*")) {
                            ...
                            targetUUID = UUID.nameUUIDFromBytes(bytes);
                        }
    
                        var attribute;
                        var rawAttribute = args[0].trim().toUpperCase().replaceAll("-", "_");
                        
                        if (rawAttribute !== "") {
                            try {
                                attribute = Attribute.valueOf(rawAttribute);
                                return formatAttributeModifiersCollection(meta.getAttributeModifiers(attribute), separator, String(attribute), nsKey, targetUUID);
                            } catch (e) {}
                        }
        
                        var slot;
                        var rawSlot = args[1].trim().toUpperCase().replaceAll("-", "_");
                        ...
                        if (rawSlot !== "") {
                            ...
                            if (!slot) {
                                try {
                                    ...
                                    return "InvalidAttributeSlot";
                                }
                            }
                        }
        
                        if (slot) {
                            if (slot instanceof EquipmentSlot) return formatAttributesMultiMap(meta.getAttributeModifiers(slot), separator, nsKey, targetUUID);
                            else {
                                var mm = meta.getAttributeModifiers();
                                var toReturn = ArrayListMultimap.create();
                                if (mm && mm.entries && !mm.isEmpty()) {
                                    ...
                                    while (it.hasNext()) {
                                        ...
                                        if (mod.getSlotGroup && mod.getSlotGroup() && mod.getSlotGroup().equals(slot)) {
                                            toReturn.put(e.getKey(), mod);
                                        }
                                    }
                                }
                                return formatAttributesMultiMap(toReturn, separator, nsKey, targetUUID);
                            }
                        }

                        if (!attribute && !slot) {
                            return "AttributeOrSlotShouldBeSpecified";
                        }
                    case "itemPickupDelay":
                        if (!entityItem) return "ItemIsNotAnEntity";
                        return entityItem.getPickupDelay();
                    case "itemOwner":
                        if (!entityItem) return "ItemIsNotAnEntity";
                        if (!entityItem.getOwner) return "Only1.16.3+"
                        return entityItem.getOwner() || "None";
                    case "itemThrower":
                        if (!entityItem) return "ItemIsNotAnEntity";
                        if (!entityItem.getThrower) return "Only1.16.3+"
                        return entityItem.getThrower() || "None";
                    case "itemUnlimitedLifetime":
                    case "itemUnlLife":
                        if (!entityItem) return "ItemIsNotAnEntity";
                        if (!entityItem.isUnlimitedLifetime) return "Only1.18.2+"
                        return entityItem.isUnlimitedLifetime();
                    default:
                        return "InvalidAction";
                }
            }
            ...
            if (identifier.startsWith("lp_has_permission_in_contexts_") || identifier.startsWith("lp_get_permission_contexts_")) {
                if (!isPluginEnabled(luckpermsPlugin)) {
                    ...
                    if (parts.length >= 2) {
                        ...
                        var result = has ? hasPermissionWithContexts(luckPerms, playerOrGroup, permission, contextString, isGroup) : getContextsForPermission(luckPerms, playerOrGroup, permission, contextString, isGroup);
                        ...
                        for (var x = startX; x <= endX; x++) {
                            for (var y = startY; y <= endY; y++) {
                                for (var z = startZ; z <= endZ; z++) {
                                    if (isExcluded(x, y, z, world.getName(), excludedCoords)) continue;
            ...
            if (identifier.startsWith("entitiesInRadius_")) {
                ...
                args[args.length - 3].split(",").forEach(function (r) {
                    if (!radiusX) {
                        radiusX = parseFloat(r);
                        return;
                    }
                    if (!radiusY) {
                        radiusY = parseFloat(r);
                        return;
                    }
                    if (!radiusZ) {
                        radiusZ = parseFloat(r);
                        return;
                    }
                });
                ...
                var entities = syncGetEntitiesInArea(world, x, y, z, radiusX, radiusY, radiusZ, include, filters, player);
            ...
            if (identifier.startsWith("entitiesListInRadius_")) {
                ...
                var radiusX, radiusY, radiusZ;
                args[args.length - 4].split(",").forEach(function (r) {
                    if (!radiusX) {
                        radiusX = parseFloat(r);
                        return;
                    }
            ...
            if (identifier.startsWith("entityAtIndexInRadius_")) {
                ...
                var radiusX, radiusY, radiusZ;
                args[args.length - 5].split(",").forEach(function (r) {
                    if (!radiusX) {
                        radiusX = parseFloat(r);
                        return;
                    }
            ...
            if (identifier.startsWith("realname")) {
                if (!isPluginEnabled(essentialsPlugin)) {
                    ...
                    switch (action) {
                        ...
                        case "divide":
                            ...
                            if (temp) {
                                ...
                                return withVarJsonLock(function (file, json) {
            ...
            if (identifier.startsWith("entity_")) {
                ...
                if (action.startsWith("path") || action.startsWith("hasPath")) {
                    try {
                        ...
                        return "OnlyPaperAndAbove";
                    }
                }
                
                var target = null;
                
                if (action.startsWith("uuidASYNC")) {
                ...
                } else if (action.startsWith("deathLoc")) {
                ...
                if (action.startsWith("location:") || action.startsWith("deathLoc:")) {
                    var parts = action.split(":");
                    action = parts[0];
                ...
                switch (action) {
                    ...
                    case "hasAI":
                        ...
                        return "TargetIsNotLivingEntity";
                    case "tags":
                    	return target.getScoreboardTags().stream().collect(Collectors.joining(separator));
                    case "target":
                    ...
                    case "hasPath":
                        if (target instanceof Mob) {
                            ...
                            return getTaskForUUID(target.getUniqueId().toString(), false, separator) !== "NoPath";
                        }
                    case "path":
                        switch (attribute) {
                            case "lastPoint":
                                if (target instanceof Mob) {
                                    ...
                                    return getTaskForUUID(target.getUniqueId().toString(), true, separator);
                                }
                                break;
                            case "allPoints":
                                if (target instanceof Mob) {
                                    ...
                                    return getTaskForUUID(target.getUniqueId().toString(), false, separator);
                                }
                                break;
                            default:
                                return "InvalidAttribute";
                        ...
                        switch (attribute) {
                            case "ASYNC":
                                ...
                                scheduler.runTaskAsynchronously(plugin, new (Java.extend(Runnable, {
                                    run: function () {
                                        try {
                                            var uuid = fetchMojangUUIDAsync(target).get(2500, TimeUnit.MILLISECONDS);
                    ...
                    case "isFlying":
                        ...
                        return "TargetIsNotPlayer";
                    case "deathLoc":
                        if (target instanceof OfflinePlayer || target instanceof HumanEntity) {
                            var output = [];
                            if (!coords) coords = ["world", "x", "y", "z", "yaw", "pitch"];

                            var location = target.getLastDeathLocation();
                            if (!location) return "NoLastDeathLocation";
                        ...
                        return "TargetIsNotOfflinePlayerOrHumanEntity"
                    case "effects":
                        if (target instanceof LivingEntity) {
                            var effects = target.getActivePotionEffects();
                            if (effects.isEmpty()) return "None";
                            
                            if (!option || option.trim() === "") return getEffectsFormatted(effects, null, separator);
                            else {
                                var certain = option.split("=");
                                var type = PotionEffectType.getByName(certain[0].trim().replaceAll("-", "_").toUpperCase());
                                if (type) {
                                    certain.shift();
                                    var arrayEffects = effects.toArray();
                                    for (var i = 0; i < arrayEffects.length; i++) {
                                        var e = arrayEffects[i];
                                        if (e.getType().equals(type)) {
                                            var output = [];
                                            certain.forEach(function (c) {
                                                c = c.trim().toUpperCase();
                                                
                                                if (c === "LVL") output.push(e.getAmplifier());
                                                if (c === "TIME") output.push(e.getDuration());
                                            });

                                            return output.length > 0 ? output.join(separator) : (e.getAmplifier() + separator + e.getDuration());
                                        }
                                    }

                                    return "EffectDoesNotExist";
                                } else {
                                    return getEffectsFormatted(effects, option.split(","), separator);
                                }
                            }
                        }
                        return "TargetIsNotLivingEntity";
                    case "hurtSound":
                        if (!(target instanceof LivingEntity)) return "TargetIsNotLivingEntity";
                        return target.getHurtSound ? (target.getHurtSound() || "None") : "Only1.19.2+";
					case "deathSound":
                        if (!(target instanceof LivingEntity)) return "TargetIsNotLivingEntity";
                        return target.getDeathSound ? (target.getDeathSound() || "None") : "Only1.19.2+";
                    default:
                        return "InvalidAction";
                }
            }
            ...
            // ===================== CHECK WHETHER THE INPUT CAN BE CASTED TO SOME INSTANCE ===================== //
            ...
            if (identifier.startsWith("replaceRegEx_")) {
                ...
                if (findDeepestPlaceholder(input)) {
                    input = resolveNestedPlaceholders(player, input);
                }
            ...
            if (identifier.startsWith("distance_")) {
                ...
                var axes = args.length > 2 ? args[2].split(",") : null;
                ...
                if (point1Identifier.indexOf(",") !== -1) {
                    ...
                    if (parts.length === 4) {
                        ...
                        try {
                        	point1 = new Location(world, x, y, z);
                        } catch (e) {
                            return "InvalidLocation";
                        }
                    } else {
                    ...
                    try {
                        ...
                        point1 = getEntitySync(uuid).getLocation();
                    } catch (e) {
                        point1 = getPlayerSync(point1Identifier, false).getLocation();
                    }
                }
                ...
                if (point2Identifier.indexOf(",") !== -1) {
                    ...
                    if (parts.length === 4) {
                        ...
                        try {
                        	point2 = new Location(world, x, y, z);
                        } catch (e) {
                            return "InvalidLocation";
                        }
                    } else {
                    ...
                    try {
                        ...
                        point2 = getEntitySync(uuid).getLocation();
                    } catch (e) {
                        point2 = getPlayerSync(point2Identifier, false).getLocation();
                    }
                }
                ...
                try {
                    if (axes) {
                        ...
                        axes.forEach(function (a) {
                        	if (a === "x") dx = point1.getX() - point2.getX();
                            else if (a === "y") dy = point1.getY() - point2.getY();
                            else if (a === "z") dz = point1.getZ() - point2.getZ();
                        });
                        
                        if (dx !== undefined && dy === undefined && dz === undefined) return Math.abs(dx);
                        else if (dx === undefined && dy !== undefined && dz === undefined) return Math.abs(dy);
                        else if (dx === undefined && dy === undefined && dz !== undefined) return Math.abs(dz);

                        var result = 0;
                        if (dx !== undefined) result += dx * dx;
                        if (dy !== undefined) result += dy * dy;
                        if (dz !== undefined) result += dz * dz;
            ...
            // ===================== GET ESSENTIALS WARPS INFORMATION ===================== //
            ...
            if (identifier.startsWith("regions_")) {
                if (!isPluginEnabled(worldguardPlugin)) {
            ...
            if (identifier.startsWith("permValue_")) {
                if (!isPluginEnabled(luckpermsPlugin)) {
                    ...
                    var result = extractMaxLimit(permission, permissionData);
                    
                    return result || "None";
                }
            }

            // ===================== GET WORLD KEY BY WORLD NAME ===================== //
            
            if (identifier.startsWith("worldKey_")) {
                var worldName = identifier.substring("worldKey_".length);
                var world = Bukkit.getWorld(worldName);

                return world ? world.getKey().toString() : "WorldDoesNotExist";
            }
            
            // ===================== PEFRORM CONSOLE COMMAND ===================== //
            
            if (identifier.startsWith("runCMD_") || identifier.startsWith("runCMD:")) {
                var silentPlaceholder, silentCMD;
                if (identifier.startsWith("runCMD:")) {
                    var subargs = identifier.split("_")[0].split(":");
                    silentPlaceholder = subargs[1] === "-s";
                    if (subargs.length > 2) {
                        silentCMD = subargs[2] === "-s";
                    }
                }
                var cmd = identifier.substring(identifier.indexOf("_") + 1);

                if (Bukkit.isPrimaryThread()) {
                    var executed = Bukkit.dispatchCommand(silentCMD && silentSender ? silentSender : consoleSender, cmd);
                    return silentPlaceholder ? "" : executed;
                }
                ...
                latch.await();
                return silentPlaceholder ? "" : result.get();
            }
            
            // ===================== PEFRORM PLAYER COMMAND ===================== //
            
            if (identifier.startsWith("runPlayer_") || identifier.startsWith("runPlayer:")) {
                if (!(identifier.startsWith("runPlayer:"))) return "EmptyPlayerName";
                    
                var silentPlaceholder;
                var subargs = identifier.split("_")[0].split(":");
                var name = subargs[1].replaceAll("-", "_");
                if (subargs.length > 2) silentPlaceholder = subargs[2] === "-s";
                
                var cmd = identifier.substring(identifier.indexOf("_") + 1);
                
                var target = getPlayerSync(name, false);
                
                if (!target) return silentPlaceholder ? "" : "InvalidPlayer";

                if (Bukkit.isPrimaryThread()) {
                    var executed = target.performCommand(cmd);
                    return silentPlaceholder ? "" : executed;
                }
            ...
            // ===================== CHECK ENDER DRAGON EXISTENCE ===================== //
            
            if (identifier.startsWith("enderDragon_")) {
                var worldName = identifier.substring("enderDragon_".length);
                var world = Bukkit.getWorld(worldName);

                if (!world) return "WorldDoesNotExist";
                if (world.getEnvironment().toString() !== "THE_END") return "WorldMustBeAnEnderWorld";
                
                return world.getEnderDragonBattle() ? world.getEnderDragonBattle().getEnderDragon() !== null : false;
            }
            
            // ===================== GET SUPERIORSKYBLOCK DIFFERNT INFORMATION ===================== //
            
            if (identifier.startsWith("superiorskyblock_")) {
                if (!isPluginEnabled(SuperiorSkyblock)) return "SuperiorSkyblockPluginIsMissing";
                
                try {
                    var SuperiorSkyblockAPI = Java.type("com.bgsoftware.superiorskyblock.api.SuperiorSkyblockAPI");
                } catch (e) {
                    return "InvalidAPIError";
                }
                
                var args = identifier.substring("superiorskyblock_".length).split("_");
                ...
                switch (option) {
                    case "islands":
                        var islands = SuperiorSkyblockAPI.getGrid().getIslands();
                        if (islands.isEmpty()) return "None";
                        
                        if (!attribute) attribute = "names,owners";
                        
                        var output = attribute.split(",");
                        var result = [];
                        
                        for (var i = 0; i < islands.size(); i++) {
                            var island = islands.get(i);
                            
                            var tempResult = [];
                            output.forEach(function (o) {
                                o = o.trim();
                                if (o === "names") tempResult.push(island.getName());
                                if (o === "owners") {
                                    var owner = island.getOwner();
                                    tempResult.push(owner ? owner.getName() : "Unknown");
                                }
                            });
