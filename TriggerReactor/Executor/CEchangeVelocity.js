...
function CEchangeVelocity() {
    var ChangeVelocityAction = Java.extend(ConditionalEventsAction, {
        execute: function(player, actionLine) {
            ...
            var entity = Bukkit.getPlayer(targetIdentifier);
            if (entity == null) {
                entity = Bukkit.getWorlds().stream()
                    .flatMap(function(world) { return world.getEntities().stream(); })
                    .filter(function(e) { return e.getUniqueId().toString().equals(targetIdentifier); })
                    .findFirst()
                    .orElse(null);
            }
