...
function followPath(targetEntity, target, pathPoints, velocitySpeed, changeView, ignoreFly, savedPoints, stuckCount, lastStuckLocation) {
    ...
    var taskId = scheduler.scheduleSyncRepeatingTask(plugin, new java.lang.Runnable({
        run: function () {
            ...
            if (points.isEmpty()) {
                ...
                savedTasksPoints.remove(uuid);
                saveData(savedTasks, savedTasksPoints);
                start(targetEntity, target, velocitySpeed, changeView, ignoreFly, savedPoints, stuckCount, lastStuckLocation);
                return;
            }
...
function start(entity, target, speed, changeView, ignoreFly, savedPoints, stuckCount, lastStuckLocation) {
    var location = entity.getLocation();
    var flying = entity.isFlying ? entity.isFlying() : !entity.hasGravity();
    var world = location.getWorld();
    try {
        var Bee = Java.type("org.bukkit.entity.Bee");
    } catch (e) {
        flying = false;
    }
    var ent = world.spawn(flying ? location.add(0, 1, 0) : world.getHighestBlockAt(location).getLocation().add(0, 1, 0), Java.type(flying ? 'org.bukkit.entity.Bee' : 'org.bukkit.entity.Zombie').class, function (e) { e.setInvisible(true); e.setVisibleByDefault(false); e.setInvulnerable(true); });
    ...
    scheduler.runTaskLater(plugin, new java.lang.Runnable({
        run: function () {
            try {
                var foundPath = pathFinder.findPath(target);
                if (!foundPath) return;
                var path = foundPath.getPoints();
                var uuid = entity.getUniqueId().toString();
                var prevTask = savedTasks.remove(uuid);
                if (prevTask != null) scheduler.cancelTask(prevTask);
                if (!cancelledStates.getOrDefault(uuid, false) && (!savedPoints || !sameBlock(savedPoints.get(savedPoints.size()-1), path.get(path.size()-1)) || !sameBlock(entity.getLocation(), path.get(path.size()-1)))) {
                    savedTasks.put(uuid, followPath(entity, target, path, speed, changeView, ignoreFly, savedPoints, stuckCount, lastStuckLocation));
                    var locList = [];
            ...
            if (entity instanceof Mob) {
                ...
                switch (action) {
                    case "SET":
                        ...
                        if (CONTINUE) {
                            ...
                            if (idTask !== -1) {
                                var prevTask = savedTasks.remove(entity.getUniqueId());
                                if (prevTask != null) scheduler.cancelTask(prevTask);
                ...
                switch (action) {
                    case "SET":
                        ...
                        start(entity, target, speed, changeView, ignoreFly, savedPoints, stuckCount, lastStuckLocation);
                        break;
                    case "RESET":
                        var uuid = entity.getUniqueId().toString();
                        cancelledStates.put(uuid, true);
                        var taskId = savedTasks.remove(uuid);
                        if (taskId != null) scheduler.cancelTask(taskId);
                        savedTasksPoints.remove(uuid);
                        saveData(savedTasks, savedTasksPoints);
                        break;
