    ...
    var ChangeTimeAction = Java.extend(ConditionalEventsAction, {
        execute: function(player, actionLine, minecraftEvent) {
            ...
            if (args.length < 2 || args.length > 5) {
                Bukkit.getLogger().warning("[CEActions] CHANGE_TIME ACTION: Invalid actionLine format! CORRECT FORMAT: change_time: <world>;<time (may be DAY, NIGHT, MIDDAY, MIDNIGHT or number from 0 to 24000)>;(optional) <player (if you need to set the time only for the specified player)>;(optional) sync (if you need to synchronize the player's time with the server);(optional) relative (if need the player's time to move and not stand still)");
                return;
            }
