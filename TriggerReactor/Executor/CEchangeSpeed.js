...
function CEchangeSpeed() {
    var ChangeSpeedAction = Java.extend(ConditionalEventsAction, {
        execute: function(player, actionLine, minecraftEvent) {
            ...
            if (args.length < 3) {
                Bukkit.getLogger().warning("[CEActions] CHANGE_SPEED ACTION: Invalid format! Correct format: change_speed: <player>;<mode (WALK or FLY)>;<speed (above -1.0 and below 1.0)>");
                return;
            }
