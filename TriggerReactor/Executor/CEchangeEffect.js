...
function CEchangeEffect() {
    var ChangeEffectAction = Java.extend(ConditionalEventsAction, {
        execute: function(player, actionLine) {
            ...
            if (target == null) {
                try {
                    var targetUUID = UUID.fromString(targetIdentifier);
                    target = Bukkit.getEntity(targetUUID);
                } catch (e) {
                    Bukkit.getLogger().warning("[CEActions] CHANGE_EFFECT ACTION: Invalid UUID format: " + targetIdentifier);
                    return;
                }
            }
